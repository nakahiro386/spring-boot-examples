...
public class ImportAdAddressConfiguration {
    ...
    public FlatFileItemReader<AdAddress> adAddressReader() {
        return new FlatFileItemReaderBuilder<AdAddress>()
                .name("adAddressReader")
                .resource(new ClassPathResource("zenkoku.csv"))
                .encoding("MS932")
                .linesToSkip(1)
                .delimited()
                .names(new String[] { "id", "kenId", "cityId", "townId", "zip", "officeFlg", "deleteFlg", "kenName",
                        "kenFuri", "cityName", "cityFuri", "townName", "townFuri", "townMemo", "kyotoStreet",
                        "blockName", "blockFuri", "memo", "officeName", "officeFuri", "officeAddress", "newId" })
        ...
        return new ItemProcessor<AdAddress, Addresses>() {
            ...
            public Addresses process(AdAddress item) throws Exception {
                ...
                Addresses address = new Addresses().withAdAddressId(item.getId())
                        ...
                        .withCityCode(item.getCityId().toString())
    ...
    public MyBatisBatchItemWriter<Addresses> addressWriter() {
        return new MyBatisBatchItemWriterBuilder<Addresses>()
                .sqlSessionFactory(sqlSessionFactory)
                .statementId(AddressesMapper.class.getName() + ".insert")
                .build();
    }
    ...
    public Step importAdAddressStep1() {
        return stepBuilderFactory.get("importAdAddressStep1")
                .tasklet(deleteAddressTasklet())
                .build();
    }

    @Bean
    public Step importAdAddressStep2() {
        return stepBuilderFactory.get("importAdAddressStep2")
                .<AdAddress, Addresses> chunk(100)
                .reader(adAddressReader())
    ...
    public Job importAdAddress() {
        return jobBuilderFactory.get("importAdAddress")
                .incrementer(new RunIdIncrementer())
                .start(importAdAddressStep1())
                .next(importAdAddressStep2())
                .build();
    }
